$(function () {	
	...
	function __getData(cb,i){
		$.post("http://localhost/getData", {i:i}, 'json').done(function (data) {
			var tmp = []; 
	...
	function __createMap(m, lat, lon){	 	
		var mapOptions = {
			...
			google.maps.event.addListener(__wrapper.map, "rightclick", function(event){	 
				$.post("http://localhost/getClosest", {lat:event.latLng.lat(), lng:event.latLng.lng(), zoom: __wrapper.map.getZoom()}, 'json').done(function(data){
					if (data.length>0){ 
